...
class Application(tk.Frame):  #定义GUI应用程序类，派生于Frame类
    def __init__(self, master=None):       #构造函数，master为父窗口
        self.files = os.listdir(r'/Users/mikeshinoda/Github/9.wallpapers') #获取图像文件名列表
        self.index = 0               #图片索引，初始显示第一张图片
        self.img = tk.PhotoImage(file=r'/Users/mikeshinoda/Github/9.wallpapers' + '/' + self.files[self.index])
